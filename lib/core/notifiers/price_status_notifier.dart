...
class PriceStatusNotifier extends StateNotifier<TickPriceStatus> {
  ...
  void compare({required double tick}) {
    ...
    if (ref.watch(previousTickListNotifier).last > ref.watch(previousTickListNotifier).reversed.elementAt(1)) {
      state = TickPriceStatus.higher;
    } else if ( ref.watch(previousTickListNotifier).last < ref.watch(previousTickListNotifier).reversed.elementAt(1)) {
